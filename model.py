...
#  ↓
# X = [sequence_length, embedding_dim]
#  ↓
# Transformer Blocks
#  ↓
# LayerNorm
...
# LM Head
#  ↓
# Next-token logits (raw scores over the vocab, highest is chosen as y-hat)


# =============================================================================
# 2. Tokenization, creating a vocabulary space
# =============================================================================
#
# === Tokenizaton, creating a vocabulary space ===
# (implemented in train.py; notes kept here for the learning path)
#
...
# =============================================================================
# 3. Token embedding, giving each token a vector representation
# =============================================================================
#
...
# embedding_dim = 4 # example size to keep it simple; in practice, the dimensions are much larger.
#
...
#
# context_len: our full context window, which is the maximum number of tokens that the model can see at once.
# same dimension as the token embeddings, so we can add them together. usually same as token embeddings for standard GPT.
...
# =============================================================================
# 5. One transformer layer (overview)
# =============================================================================
#
...
# =============================================================================
# 6. Causal self-attention (concept)
# =============================================================================
#
# what is causal self attention for? given 2 sentences: 
# 1. this is a river bank
# 2. i went to withdraw money from the bank
...
#
# self = based on the same input source (Q, K and V come from the same source)
...
# =============================================================================
#
# A single attention head captures 1 way to compare tokens.
# MinGPT uses several of these inside MultiHeadAttention to capture multiple ways to compare tokens (see section 8).
# implementation of a single attention head
#
# learned matrix: embedding_dim input features → head_size output features
...
# 1 1 1 0
# 1 1 1 1
# do not pay attention to future tokens.
#
...
# =============================================================================
# 10. Layer normalization + Feed forward network + Transformer block
# =============================================================================
...
# =============================================================================
#
# === Stacking transformer blocks to form the transformer ===
# Why?
...
class MinGPT(nn.Module):
    def __init__(self, vocab_size, embedding_dim, context_len, num_heads, num_layers):
        ...
        # Convert hidden representation into next-token logits (raw scores over the vocab), converted to probabilities using softmax in later step.
    ...
    def forward(self, token_ids):
        ...
        position_ids = torch.arange(T, device=token_ids.device)  # same device eg, gpu/cpu as inputs

        token_embeddings = self.token_embedding(token_ids)
        position_embeddings = self.position_embedding(position_ids)

        X = token_embeddings + position_embeddings
        # compute attention via the transformer block, which also captures the residual
        ...
        # Note: loss computation usually uses the logits directly, not the probabilities, since the
        # pytorch functions internally handle computing numerical stable version of probability representation.
